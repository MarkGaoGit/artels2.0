...
class usercontrol extends bases {
	...
	function onregister() {
		...
		$uid = $_ENV['user']->add_user($username, $password, $email, 0, $questionid, $answer, $regip, $salt);
		return $uid;
	}
	...
	function onget_user() {
		...
		if($status) {
            return array($status['uid'],$status['username'],$status['email'],$status['salt'],$status['password']);
		} else {
			return 0;
		}
	}
