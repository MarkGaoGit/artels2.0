...
function uc_api_mysql($model, $action, $args=array()) {
    ...
    if(empty($uc_controls[$model])) {
        include_once DOC_ROOT. 'uc_client/lib/db.class.php';
        include_once DOC_ROOT. 'uc_client/model/bases.php';
        /*
         * 加上了绿云的订单  艺术品的订单 绿云用户信息
         * table：uc_ly_order uc_ly_integral uc_crm_message uc_artels_order
         * auth : gaokang
         * time : 2016-11-21 11:07
         * */
//        include_once DOC_ROOT. 'uc_client/control/user.php';
        include_once DOC_ROOT. "uc_client/control/$model.php";
...
function uc_user_register($username, $password, $email, $questionid = '', $answer = '', $regip = '', $salt) {
    return call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password, 'email'=>$email, 'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip, 'salt' => $salt));
}
 ...
 * time：2016-11-21
 * */
function uc_lvyun_order($params){
    return call_user_func(UC_API_FUNC, 'lvyun' , 'add' , $params);
}

function uc_artels_order($params){
    return call_user_func(UC_API_FUNC, 'artels' , 'add' , $params);
}

function uc_lvyun_integral($params){
    return call_user_func(UC_API_FUNC, 'integral' , 'add' , $params);
}

function uc_crm_message($params){
    return call_user_func(UC_API_FUNC, 'crm' , 'add' , $params);
}
