...
class usermodel {
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '', $salt) {
	...
	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
        ...
        $ypwd = md5($data['password'].$data['salt']);

		if(!$ignoreoldpw && $ypwd != $oldpw) {
			return -1;
		}

		$sqladd = $newpw ? "password='".$newpw."'" : '';
