    ...
    <ul class="mui-table-view layout-list-common margin-none">
    	<li class="mui-table-view-cell">
    		<div class="full mui-pull-left padding-bottom border-bottom">
    			<span class="icon-15 mui-pull-left margin-small-right"><img src="<?php echo SKIN_PATH;?>statics/images/ico_22.png" /></span>请选择并确认您的收货地址
    		</div>
    		<div class="mui-clearfix"></div>
<a href="<?php echo url('order/order/settlement_address',array('skuids' => $_GET['skuids']));?>" class="mui-navigate-right settlement-address">
...
<li class="mui-table-view-cell">
<a href="<?php echo url('order/order/settlement_delivery');?>" class="mui-navigate-right">
...
<li class="mui-table-view-cell">
<a href="<?php echo url('order/order/settlement_invoice');?>" class="mui-navigate-right">
<span class="hd-h4">发票信息</span>
<span class="mui-pull-right" data-show="invoice_content">请填写</span>
</a>
</li>
<?php } ?>
</ul><?php if(is_array($carts['skus'])) foreach($carts['skus'] as $sellerid => $cart) { ?><ul data-sellerid="<?php echo $sellerid;?>" class="custom-goods-items custom-goods-row row1 custom-list-goods list-col-10 mui-clearfix">
<li class="padding border-bottom" >
<span >运费：￥<em data-show="delivery_price"><?php echo $cart['delivery_price'];?></em></span>
</li>
...
<?php if(($cart['_promos'])) { ?>
<a href="<?php echo url('order/order/settlement_order',array('sellerid' => $sellerid));?>" class="padding-tb border-top prom-nav text-drak mui-block mui-navigate-right">
    			<span class="hd-h5">订单促销</span>
<span class="mui-pull-right margin-big-right" data-show="order_prom">请选择</span>
    		</a>
    		<?php } ?>
</li><?php if(is_array($cart[sku_list])) foreach($cart[sku_list] as $v) { ?>    	<li class="goods-item-list" data-skuid="<?php echo $v['_sku_']['sku_id'];?>">
<div class="list-item">
<div class="list-item-pic">
<a href="<?php echo url('goods/index/worksDetail',array('sid' => $v[_sku_][spu_id]));?>"><img src="<?php echo $v['_sku_']['thumb'];?>"></a>
</div>
<div class="list-item-bottom">
<div class="list-item-title">
<a href="<?php echo url('goods/index/worksDetail',array('sid' => $v[_sku_][spu_id]));?>"><?php echo $v['_sku_']['sku_name'];?></a>
</div>
<div class="list-item-text hd-h6 mui-row">
<span class="text-ellipsis text-gray mui-col-xs-6"><?php if(is_array($v[_sku_][spec])) foreach($v[_sku_][spec] as $spec) { ?><?php echo $spec['name'];?>：<?php echo $spec['value'];?>　
<?php } ?>
</span>
<span class="mui-text-right mui-col-xs-6"><em class="price-org hd-h4">￥<?php echo $v['_sku_']['prom_price'];?></em>× <?php echo $v['number'];?></span>
</div>
</div>
</div>
<?php if(($v[_promos])) { ?>
<a href="<?php echo url('order/order/settlement_goods',array('sellerid' => $sellerid ,'skuid' => $v[_sku_][sku_id]));?>" class="padding-tb margin-lr-15 border-top prom-nav text-drak mui-block mui-navigate-right">
...
<a href="<?php echo url('order/cart/index');?>" class="go-car">返回购物车</a>
<div class="cart-footer-box full">
    <p class="mui-pull-right cart-total mui-text-right">
<span class="text-org">合计：￥<em class="normal" data-show="real_amount"><?php echo $carts['real_amount'];?></em></span><br/>
结算商品数量：<em class="normal" data-show="sku_numbers"><?php echo $carts['sku_numbers'];?></em> 件
</p>
</div>
...
referer : '<?php echo url("order/order/settlement" ,array("skuids" => $_GET["skuids"]));?>',	
};
