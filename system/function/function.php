...
function runhook($hookid, $params,$flag = FALSE) {
    if($flag == TRUE){
        return Hook::execute($hookid, $params,$flag);
    }else{
        Hook::execute($hookid, $params);
    }
}
 ...
 * @return boolean
 * **/
function is_idno($string){
    if (!empty($string)) {
        return preg_match('/^\d{18}$/', $string);
    }
    return FALSE;
}
