...
class Input{
    ...
    private static function filter(&$value, $key, $filters)
    {
        ...
        foreach ($filters as $filter) {
            if (is_callable($filter)) {
                ...
                if (in_array($begin, array('/', '#', '~')) && $begin == $end = substr($filter, -1)) {
                    // 正则过滤
    ...
    {
        return strpos($name, '/') ? explode('/', $name, 2) : array($name, 's');
    }
    ...
    private static function getFilter()
    {
        if (is_null(static::$filters)) {
            $filters         = '';
            static::$filters = empty($filters) ? array() : (is_array($filters) ? $filters : explode(',', $filters));
        }
