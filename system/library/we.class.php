...
class we
{
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $this->salt_type = isset($_GET["salt_type"]) ? $_GET["salt_type"]: '';
            if ($this->salt_type == 'aes') { //aes加密
                $this->log($postStr);
            	$saltStr = $array['salt'];
            	$pc = new Prpcrypt($this->encodingAesKey);
            	$array = $pc->decrypt($saltStr,$this->appid);
        ...
        if (!$this->checkSignature($saltStr)) {
        	if ($return)
        		return false;
        	else
        		die('no access');
	...
	*/
	...
	public function reply($msg=array(),$return = false)
	{
		...
		$this->log($xmldata);
		if ($this->salt_type == 'aes') { //如果来源消息为加密方式
		    $pc = new Prpcrypt($this->encodingAesKey);
		    $array = $pc->salt($xmldata, $this->appid);
		    $ret = $array[0];
		    if ($ret != 0) {
		        $this->log('salt err!');
		        return false;
		    ...
		    $salt = $array[1];
		    $tmpArr = array($this->token, $timestamp, $nonce,$salt);//比普通公众平台多了一个加密的密文
		    sort($tmpArr, SORT_STRING);
		    $signature = implode($tmpArr);
		    $signature = sha1($signature);
		    $xmldata = $this->generate($salt, $signature, $timestamp, $nonce);
		    $this->log($xmldata);
		}
		if ($return)
			return $xmldata;
     ...
     * xml格式加密，仅请求为加密方式时再用
     */
	private function generate($salt, $signature, $timestamp, $nonce)
	{
	    //格式化加密信息
	    $format = "<xml>
<salt><![CDATA[%s]]></salt>
...
<Nonce><![CDATA[%s]]></Nonce>
	    ...
	    return sprintf($format, $salt, $signature, $timestamp, $nonce);
	}
    ...
    /**
     * code 解码
     * @param string $salt_code 通过 choose_card_info 获取的加密字符串
     * @return boolean|array
     * {
     *  "errcode":0,
     *  "errmsg":"ok",
     ...
     *  }
     */
    public function decryptCardCode($salt_code) {
        $data = array(
            'salt_code' => $salt_code,
        );
        if (!$this->access_token && !$this->checkAuth()) return false;
...
class Prpcrypt
{
     ...
     */
    public function salt($text, $appid)
    {
        ...
        try {
            ...
            //加密
            $salted = mcrypt_generic($module, $text);
            mcrypt_generic_deinit($module);
            mcrypt_module_close($module);

            //			print(base64_encode($salted));
            //使用BASE64对加密后的字符串进行编码
            return array(ErrorCode::$OK, base64_encode($salted));
        } catch (Exception $e) {
            //print $e;
            return array(ErrorCode::$saltAESError, null);
        }
    }

    /**
     * 对密文进行解密
     * @param string $salted 需要解密的密文
     * @return string 解密得到的明文
     */
    public function decrypt($salted, $appid)
    {

        try {
            //使用BASE64对需要解密的字符串进行解码
            $ciphertext_dec = base64_decode($salted);
            $module = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
            $iv = substr($this->key, 0, 16);
            mcrypt_generic_init($module, $this->key, $iv);
            //解密
...
class ErrorCode
{
    ...
    public static $saltAESError = 40006;
