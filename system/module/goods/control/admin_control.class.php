...
class admin_control extends init_control {
	...
	public function index_hotel(){
		...
		$this->load->librarys('View')->assign('brands',$brands);
		$this->load->librarys('View')->assign('pages',$pages);
		$this->load->librarys('View')->display('goods_hotel_list');
	}
	...
	public function goods_add(){
		...
		if(checksubmit('dosubmit')) {
			switch($_GET['web_style']){
				case '1':
					$_GET['top'] = $_GET['img_top'] = $_GET['left'] = $_GET['is_bg'] = 0;
					break;
				case '2':
					$_GET['top'] = '-222px';
					$_GET['img_top'] = '160px';
					$_GET['left'] = $_GET['is_bg'] = 0;
					break;
				case '3':
					$_GET['img_top'] = $_GET['left'] = '-20px';
					$_GET['top'] = '-220px';
					$_GET['is_bg'] = 1;
					break;
				case '4':
			...
			if($result === false){
				...
				if($step == 4){
					if($cache['base']['is_hotel'] == 1){
						showmessage(lang('_operation_success_'),url('index_hotel'));
					}else{
						showmessage(lang('_operation_success_'),url('index'));
					}
				}else{
			...
			$roomCode = $this->load->table('room_code')->select();

			$this->load->librarys('View')->assign('rcode',$roomCode);
			$this->load->librarys('View')->assign('info',$info);
	...
	public function ajax_del(){
		...
		if(!$result){
			...
			if($_GET['is_hotel']){
				showmessage(lang('_operation_success_'),url('index_hotel'),1);
			}else{
