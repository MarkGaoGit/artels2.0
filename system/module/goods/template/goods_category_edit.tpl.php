...
<div class="content padding-big have-fixed-nav">
	<form action="" method="POST">
		<div class="form-box">
			<?php echo Form::input('text', 'name', $cate['name'], '分类名称：','填写分类名称，不能为空',array('validate' => 'required')); ?>
		</div>
		...
		<div class="padding-lr">
			<div class="form-group">
				...
				<div class="box ">
					<div class="field">
						<input class="goods-class-text input input-readonly" id="choosecat" value="<?php echo $cate['parent_name'] ? $cate['parent_name'] : $parent_name?>" title="" readonly="readonly" type="text" placeholder="请选择分类" data-reset="false" />
				...
				<p class="desc">选择分类隶属哪个分类下，如是顶级分类，请直接选择顶级分类</p>
			</div>
			<div class="form-group">
				<span class="label">关联类型：</span>
				<div class="box ">
					<div class="field">
						<input class="goods-class-text input input-readonly" id="choosetype" value="<?php echo $cate['type_name']?>" title="" readonly="readonly" type="text" placeholder="请选择类型" data-reset="false" />
				...
				<p class="desc">每个分类可以管理一个类型，用于前台筛选及类型添加</p>
			</div>
		</div>
		<div class="form-box clearfix">
			<?php echo Form::input('hidden','url',$cate['url'] ? $cate['url'] : 'http://','分类外链：','为前台分类列表指定一个固定连接用于跳转，不影响分类的基本属性，特殊情况下使用，否则无需填写')?>
			<?php echo Form::input('text','grade', $cate['grade'],'英文名称：','英文名称适用于酒店'); ?>
			<?php echo Form::input('radio','is_hotel', isset($cate['is_hotel']) ? $cate['is_hotel'] : '0','是否是酒店分类：','是否是酒店分类 默认 否',array('items' => array('1'=>'是','0'=> '否'), 'colspan' => 2,)); ?>
			<?php echo Form::input('text', 'sort', isset($cate['sort']) ? $cate['sort']:'100','排序：','请填写自然数。分类将会根据排序进行由小到大排列显示'); ?>
			<?php echo Form::input('enabled','status',isset($cate['status']) ? $cate['status'] : '1','是否启用：','请设置分类是否启用，不影响后台显示使用', array('colspan' => 2)); ?>
			<?php echo Form::input('enabled','show_in_nav',isset($cate['show_in_nav']) ? $cate['show_in_nav'] : '0','是否显示在导航：','请设置酒店分类是否在前台导航上推荐', array('colspan' => 2)); ?>
