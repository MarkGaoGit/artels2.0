...
class goods_spu_service extends service {
	...
	public function get_lists($params){
		...
		if($params['label'] == 1 || $params['label'] == FALSE){
			...
			if($params['is_hotel'] || $params['is_hotel'] == 0){
				$goods_sql = $this->spu_db->status($params['status'])->is_hotel($params['is_hotel'])->category($params['catid'])->brand($params['brand_id']);
			}else{
			...
			if($params['keyword']){
				...
				if($params['is_hotel'] || $params['is_hotel'] == 0){
					$count = $this->load->table('goods/goods_spu')->status($params['status'])->is_hotel($params['is_hotel'])->category($params['catid'])->brand($params['brand_id'])->where($keyword_sql)->count();
				}else{
				...
				if($params['is_hotel'] || $params['is_hotel'] == 0){
					$count = $this->load->table('goods/goods_spu')->status($params['status'])->is_hotel($params['is_hotel'])->category($params['catid'])->brand($params['brand_id'])->count();
				}else{
			...
			$sqlmap = array();
			if($params['is_hotel'] || $params['is_hotel'] == 0){
				$sqlmap[config("DB_PREFIX").'goods_spu.is_hotel'] = $params['is_hotel'];
			}
			switch ($params['label']) {
				case '2':
