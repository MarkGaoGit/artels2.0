...
class spec_service extends service {
	...
	public function add_spec($params){
		if(empty($params['value'])){
			$this->error = lang('goods/goods_spec_value_empty');
			return FALSE;
		}
	...
	public function ajax_add_spec($name){
		if (empty($name)) {
			$this->error = lang('goods/goods_spec_name_require');
			return FALSE;
		}
		$result = $this->db->add(array('name'=>$name));
		if (!$result) {
			$this->error = lang('goods/goods_spec_add_fail');
		}else{
	...
	public function ajax_add_pop($params){
		...
		if ($data['id'] < 1){
			$this->error = lang('goods/goods_spec_id_error');
		} 
		...
		if (empty($old_info['value'])) {
			...
			foreach ($old_spec AS $spec_value) {
				if($new_value == $spec_value){
					$this->error = lang('goods/spec_exist');
					return FALSE;
				}
			}
			$old_spec[] = $new_value;
		...
		if ($result === FALSE){
			$this->error = lang('goods/goods_pop_add_fail');
		}else{
