...
{	
	...
	public function index() {
		...
		$nocomment = $this->counts['load_comment'];
		extract($result);
		...
		$this->load->librarys('View')->assign('iscomment',$iscomment);
		$this->load->librarys('View')->assign('nocomment',$nocomment);
		$this->load->librarys('View')->display('comment');
	}
	...
	public function add() {
		...
		if(checksubmit('dosubmit')) {
			...
			showmessage(lang('comment/publish_estimate_success'), url('index'), 1);
		} else {
	...
	public function set_comment(){
		...
		$this->load->librarys('View')->display('set_comment');
	}
	...
	public function comment_detail(){
		...
		$this->load->librarys('View')->assign('result',$result);
		$this->load->librarys('View')->assign('SEO',$SEO);
		$this->load->librarys('View')->display('comment_detail');
	}
