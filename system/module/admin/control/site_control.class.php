...
class site_control extends init_control{
    ...
    public function base() {
        ...
        if(checksubmit('dosubmit')) {
			...
			$this->load->librarys('View')->display('site_base');
        }
    }
	/* 注册与访问 */
    public function reg() {
      if(checksubmit('dosubmit')){
			...
			$setting = $this->load->table('setting')->getField('key, value', TRUE);
			$this->load->librarys('View')->assign('setting',$setting);
			$this->load->librarys('View')->display('site_reg');
		}
    }
    ...
    public function seo() {
    	if(checksubmit('dosubmit')){
			...
			$this->load->librarys('View')->assign('seos',$seos);
			$this->load->librarys('View')->display('site_seo');
		}
    }
    ...
    public function rewrite() {
        ...
        $this->load->librarys('View')->assign('rewrites',$rewrites);
		$this->load->librarys('View')->display('site_rewrite');
	}
    ...
    public function ajax_update_rewrite() {
    	...
    	$rewrites = @include CONF_PATH.'rewrite.php';
    	if (!$rewrites[$id]) showmessage(lang('admin/record_no_exist'));
