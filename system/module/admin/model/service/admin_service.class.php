...
class admin_service extends service {
    ...
    public function login($username, $password) {
       if(empty($username)) {
        ...
        if($admin_user['password'] !== $this->_create_password($password, $admin_user['salt'])) {
     ...
     * @return string
     */
    public function _create_password($pwd, $salt = '') {
	...
	public function update($params,$files){
		...
		$data['id'] = ADMIN_ID;
		$data['salt'] = random(10);
		$data['password'] = create_password($params['newpassword'], $data['salt']);
