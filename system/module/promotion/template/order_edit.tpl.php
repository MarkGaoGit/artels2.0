...
<div class="content padding-big have-fixed-nav">
	<form action="<?php echo url('edit');?>" name="promotion_order_add" method="post">
	<input type="hidden" name="id" value="<?php echo $info['id'] ?>">
	<div class="form-box clearfix">
		<?php
		$discount[0] = $discount[1] = $discount[2] = 0;
		$discount[$info['type']] = $info['discount'];
		?>
		...
		<?php echo Form::input('select', 'type', $info['type'], '促销类型：', '请选择促销类型', array(
			'items' => array('满额立减', '满额免邮', '满额赠礼'),
		)); ?>
		<?php echo Form::input('text', 'price', $info['price'], '单笔订单满足金额：', '请填写单笔订单满足金额', array("datatype" => 'price')); ?>

		<div class="prom-list <?php if ($info['type'] != 0): ?>hidden<?php endif ?>" data-type="0">
			<?php echo Form::input('text', 'discount[0]', $discount[0], '单笔订单立减金额：', '单笔订单满足条件后，订单减少的金额'); ?>
		</div>

		<div class="prom-list <?php if ($info['type'] != 1): ?>hidden<?php endif ?>" data-type="1">
			<input type="hidden" name="discount[1]" value="0">
		</div>

        <div class="prom-list <?php if ($info['type'] != 2): ?>hidden<?php endif ?>" data-type="2">
    		<div class="form-group">
				<span class="label">赠送礼品：</span>
				<div class="box">
					<input class="goods-class-text input input-readonly" value="<?php echo $info['_sku_info_']['sku_name'] ?>" readonly="readonly" type="text" data-title="" data-pic="" data-price="" data-spec="" data-number="" data-id="<?php echo $discount[2] ?>" data-sku="true"/>
					<input type="hidden" name="discount[2]" value="<?php echo $discount[2] ?>" data-type="id">
