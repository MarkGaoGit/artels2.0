...
function install_sql(){
	...
	//检测SQL安装文件
	$sql_file = MODULE_PATH.'sql/table.sql';
	...
	if(!file_exists($sql_data_file))
	{
		...
		showProgress(array('isError' => false,'message' => '正在加载必要数据','percent' => 90.0));
		$sqls = parseSQL($sql_data_file);
		execSQL($sqls);
		showProgress(array('isError' => false,'message' => '更新必要数据成功','percent' => 99.6));
	}
...
EOF;
	...
	$adminSql = 'insert into `'.$db_pre.'admin_user` (`username`,`password`,`email`,`salt`,`group_id`) values ("'.$admin_user.'","'.md5($admin_pwd.$valid).'","'.$admin_email.'","'.$valid.'",1)';
	if(!mysql_query($adminSql))
	{
		showProgress(array('isError' => true,'message' => '创建管理员失败'.$adminSql.mysql_error(),'percent' => 99.8));
	}else{
		showProgress(array('isError' => false,'message' => '创建管理员成功','percent' => 99.8));
	}
	...
	if(!mysql_query($adminSql))
	{
		showProgress(array('isError' => true,'message' => '更新站点配置失败'.$adminSql.mysql_error(),'percent' => 99.8));
	}else{
		showProgress(array('isError' => false,'message' => '更新站点配置成功','percent' => 99.8));
	}
	...
	showProgress(array('isError' => false,'message' => '更新站点缓存成功','percent' => 99.9));
