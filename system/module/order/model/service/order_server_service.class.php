...
class order_server_service extends service {
	...
	public function create_return ($o_sku_id ,$amount = 0 ,$cause , $desc = '',$images = array()) {
		...
		if ($amount <= 0) {
			$this->error = lang('order/refund_money_require');
			return FALSE;
		}
		if (!$cause) {
			$this->error = lang('order/return_cause_empty');
			return FALSE;
		}
		...
		if ($check_has > 0) {
			$this->error = lang('order/repeat_submit');
			return FALSE;
		}
	...
	public function handle_return($id ,$status ,$msg = '') {
		...
		if (!$info) {
			$this->error = lang('order/operate_record_not_exist');
			return FALSE;
		}
		if (in_array($info['status'], array(-2,-1,2))) {
			$this->error = lang('order/record_ban_operate');
			return FALSE;
		}
		$data = array();
		$data['id'] = $id;
	...
	public function handle_refund($id ,$status = 0 ,$msg = '') {
		...
		if ($status == 1) {
			...
			$ds = $this->table->where(array('refund_id' => $id))->setField('status',2);
		}else{
			$ds = $this->table->where(array('refund_id' => $id))->setField('status',-2);
		}
		// 退款日志
	...
	public function return_goods($id ,$name ,$sn) {
		...
		$sn   = (string) trim($sn);
		if (empty($name)) {
			$this->error = lang('order/logistics_name_empty');
			return FALSE;
		}
		if (empty($sn)) {
			$this->error = lang('order/waybill_sn_empty');
			return FALSE;
		}
