...
class cart_service extends service {
	...
	public function set_nums($sku_id ,$nums ,$buyer_id = 0) {
		...
		if ($sku_id < 1) {
			$this->error = lang('order/buy_number_require');
			return FALSE;
		}
		if ($buyer_id == 0) {	// 游客更新cookie
			...
			if (empty($all_item[$sku_id])) {
				$this->error = lang('order/goods_not_exist');
				return FALSE;
			}
			if ($nums == 0) {	// 删除该记录
	...
	public function delpro($sku_id ,$buyer_id = 0) {
		...
		$buyer_id = (int) $buyer_id;
		if (!$sku_id) {
			$this->error = lang('order/delete_parame_empty');
			return FALSE;
		}
		if ($buyer_id > 0) {
			$sqlmap = array();
	...
	public function dec_nums($params = array() ,$buyer_id = 0) {
		...
		if (empty($params) || empty($sku_ids)) {
			$this->error = lang('order/parameter_empty');
			return FALSE;
		}
		$sqlmap = array();
