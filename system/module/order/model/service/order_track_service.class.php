...
class order_track_service extends service {
	...
	public function add($order_sn ,$sub_sn ,$msg = '' ,$time = 0 ,$delivery_id = 0) {
		...
		if (!$order_sn) {
			$this->error = lang('order/parent_order_sn_empty');
			return FALSE;
		}
		if (!$sub_sn) {
			$this->error = lang('order/child_order_sn_empty');
			return FALSE;
		}
		$data = array();
	...
	public function update_api100 ($sub_sn,$o_d_id) {
		...
		if (!$o_d_info) {
			$this->error = lang('order/order_logistics_not_exist');
			return FALSE;
		}
        if(!$o_d_info['delivery_sn']){
        	$this->error = lang('order/waybill_sn_not_exist');
        	return FALSE;
        }
        // 获取物流标识
       	$identif = $this->table_delivery->getFieldById($o_d_info['delivery_id'] ,'identif');
       	if (!$identif) {
       		$this->error = lang('order/deliver_identify_not_exist');
       		return FALSE;
       	}
	...
	public function kuaidi100($com , $nu) {
		if(empty($com) || empty($nu)) {
			$this->error = lang('order/submit_parameters_error');
			return FALSE;
		}
		...
		if($result) {
			...
			if ($result['status'] == 200) {
				...
				$this->error = lang('order/connector_error');
				return FALSE;
			} else {
				$this->error = lang('order/logistics_no_msg');
				return FALSE;
			}
		} else {
			$this->error = lang('order/inquire_error');
			return FALSE;
		}
	}
}