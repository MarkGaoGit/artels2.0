	...
	<div class="table-wrap member-info-table">
		<div class="table resize-table paging-table check-table high-table border clearfix">
			<div class="tr">
				...
				<?php foreach ($members['table'] AS $tr): ?>
				<span class="th" data-width="<?php echo $tr['width'] ?>"><span class="td-con"><?php echo $tr['name'] ?></span></span>
				<?php endforeach ?>
				<span class="th" data-width="10" style="width: 183px;"><span class="td-con">状态</span></span>
			</div>
			...
			<div class="tr">
				...
				<span class="td">
					<span class="td-con double-row text-left">会员卡号：<?php echo $member['cardNo'] ?><br />身份证号：<?php echo $member['id_no']?></span>
				</span>
				<span class="td">
				...
				<span class="td"><?php echo ($member['islock'] == 1) ? '锁定' : '正常' ?></span>
					<span class="td">
					<span class="td-con"><a href="javascript:;" class="member_update" data-url="<?php echo url('update', array('id' => $member['id'])) ?>">编辑</a>&nbsp;&nbsp;&nbsp;<a href="<?php echo url('delete', array('ids[]' => $member['id'])); ?>" data-confirm="您确认要删除？">删除</a></span>
				</span>
			</div>
			<?php endforeach ?>
