...
class account_control extends cp_control
{
	...
	public function resetpassword(){
		if(IS_POST) {
			...
			$oldpwd = md5(md5($oldpassword) . $this->member['salt']);
			if ($oldpwd !== $this->member['password']) {
				echo json_encode('旧密码不正确，请重新输入');
				exit;
			}
			if ($newpassword !== $newpassword1) {
				showmessage(lang('member/two_passwords_differ'), '', 0);
			}
			$data['password'] = md5(md5($newpassword) . $this->member['salt']);
			...
			if ($lvyun_result['resultCode'] != 0) {
				echo json_encode($lvyun_result['reusltMsg']);
				exit;
			} else {
				//UC
				include_once DOC_ROOT . 'config.inc.php';
				include_once DOC_ROOT . 'uc_client/client.php';
				$ucpwd = uc_user_edit($this->member['username'], $oldpwd, md5($newpassword));
