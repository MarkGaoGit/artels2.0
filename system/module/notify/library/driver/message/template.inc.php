<p class="notice">您正在编辑 <em id="content-label" class="text-main">loading</em> 通知模板</p>
	...
	<div id='edit_<?php echo $tk?>' style="display: none;" class="layout clearfix">
		<div class="form-layout-rank clearfix">
			<?php echo Form::input('text', "{$tk}[title]", "{$template[template][$tk]['title']}", '短信标题：', '标题支持标签请从编辑器中复制');?>
		</div>
		<?php echo Form::editor("{$tk}[content]", "{$template[template][$tk]['content']}", '', '', array('mid' => $this->admin['id'], 'path' => 'notify'),"{$tk}",TRUE); ?>
