		...
		cart_add : function(obj) {
			...
			$.ajax({
				...
				success:function(ret) {
					var success_url = '?m=order&c=cart&a=index';
					var cart_url = '?m=goods&c=index&a=worksDetail&sku_id='+sku_ids;
					if(ret.status == 1){
						hd_cart._counts();
						if(cart_jump == 0){
			...
			$.getJSON('?m=order&c=cart&a=get_carts',{group:'TRUE',format:'json'}, function(ret){
				...
				if (ret.sku_counts > 0) {
					...
					$.each(ret.skus,function(k, v) {
						...
						_html += '<div class="th">';
						_html += '	<span class="th-check"><label><input class="seller_box" type="checkbox" checked="checked" /></label></span><span style="margin-left:-440px;">名称</span>';
						...
						_html += '	<span class="th-hand text-center">操作</span></div>';
						$.each(v.sku_list,function(key ,cart){
							...
							if(cart._sku_.brand && cart._sku_.spec != ''){
								_html += '				<p class="text-ellipsis text-gray-666"><a class="text-gray-666 aho" href="?m=goods&c=index&a=worksDetail&sku_id='+ cart._sku_.sku_id +'" title="'+ cart._sku_.name +'" target="_blank">'+ prom + cart._sku_.brand.name +'——'+ cart._sku_.name +'</a><br/>尺寸：'+getsStr(cart['_sku_']['spec'][0]['value'],'/')+'</p>';
							}else{

								_html += '				<p class="text-ellipsis text-gray-666"><a class="text-gray-666 aho" href="?m=goods&c=index&a=worksDetail&sku_id='+ cart._sku_.sku_id +'" title="'+ cart._sku_.name +'" target="_blank">'+ prom + cart._sku_.name +'</p>';
							}
									if (parseInt(cart._sku_.number) == 0) {
										_html += '	<span>已售罄</span>';
									}
							...
							_html += '			<p class="shop_price text-gray-666">'+ cart._sku_.prom_price +'</p>';
							_html += '		</div>';
							...
							_html += '			<span class="text-mix text-big-small">'+ cart.prices +'</span>';
							_html += '		</div>';
							_html += '		<div class="cell cart-hand text-center">';
							_html += '			<a class="delpro text-gray-666 aho"  href="javascript:;">删除</a>';
							_html += '		</div>';
							_html += '	</div>';
							_html += '</div>';
						});
